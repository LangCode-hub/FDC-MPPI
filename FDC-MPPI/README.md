...
The release provides the FDC-MPPI local obstacle-avoidance simulation used with
...
FDC-MPPI/
├── src/
│   ├── fr_mhe_estimation/      # FR-MHE disturbance-coefficient estimator
│   ├── go2_costmap_demo/       # LaserScan -> local costmap and EDT
│   ├── robot_state_collector/  # Robot-state and command logger
│   ├── unitree_guide/
...
- `src/fr_mhe_estimation/src/fr_mhe_node.py`
- `src/fr_mhe_estimation/src/gradient_calculator.py`
...
- Eigen3, OpenCV 4, LCM, libxlsxwriter, NumPy, and the ROS packages declared in
  each `package.xml`
...
sudo apt update
sudo apt install -y \
  build-essential cmake libeigen3-dev liblcm-dev libopencv-dev libxlsxwriter-dev \
  python3-numpy \
...
Use four terminals. Source ROS and the workspace in every terminal.
...
The supplied launch file contains an RViz node with an author-specific
configuration path. Update that path or comment out the RViz node if the file
is unavailable on your machine.
...
Terminal 3 — run FR-MHE to estimate disturbance-compensation coefficients:

```bash
cd FDC-MPPI
source /opt/ros/noetic/setup.bash
source devel/setup.bash
rosrun fr_mhe_estimation fr_mhe_node.py
```
...
Focus Terminal 4 and use the following keys:
...
The default navigation goal is `(5.1, 2.0)` metres. Change it in
...
| `/robot/mu_estimation` | FR-MHE disturbance-compensation coefficients |
...
- Start the FR-MHE node before enabling FDC-MPPI so that compensation
  coefficients are available to the controller.
- To record synchronized robot states and control commands in
  `robot_state_data.xlsx`, run `rosrun robot_state_collector state_collector`.
