...
#include <geometry_msgs/Twist.h>  // Publishes velocity control commands.
...
private:
    ...
    ros::Publisher mppi_control_pub_;
    double ctrl_vx_global_ = 0.0;
    double ctrl_vy_global_ = 0.0;
    double ctrl_wz_global_ = 0.0;

    // Custom message (geometry_msgs/TwistWithCovariance could also be used).
    //ros::Publisher global_control_pub_;
    // Store the latest MPPI control command.
    Control latest_mppi_control_;
    std::mutex control_mutex_;  // Protect access to the control command.
};
